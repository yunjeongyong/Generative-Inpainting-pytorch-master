...
class CoarseNet(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):  # 5, 32
        super(CoarseNet, self).__init__()
        self.down = Down_Module(in_ch, out_ch)  # (5, 32)
        self.atrous = Dilation_Module(out_ch * 4, out_ch * 4)  # (128, 128)
        self.up = Up_Module(out_ch * 4, 3)  # (128, 3)
...
class RefinementNet(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.down_conv_branch = Down_Module(in_ch, out_ch)  # (5, 32)
        self.down_attn_branch = Down_Module(in_ch, out_ch, activation=nn.ReLU())  # (5, 32, ReLU)
        self.atrous = Dilation_Module(out_ch * 4, out_ch * 4)  # (128, 128)
        self.CAttn = Contextual_Attention_Module(out_ch * 4, out_ch * 4)  # (128, 128)
        self.up = Up_Module(out_ch * 8, 3, isRefine=True)  # (256, isRefine=True)
